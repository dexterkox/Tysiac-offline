...
function krok(stan, rand) {
  ...
  while (bezpiecznik++ < 500) {
    ...
    /* --- odkryty musik: widzą go WSZYSCY i każdy potwierdza, że obejrzał --- */
    if (stan.phase === 'musikreveal') {
      if (!stan.musikPokazany) {
        /* Karty trafiają do ręki grającego, ale zostają na stole, dopóki wszyscy nie
           potwierdzą — inaczej przeciwnik nie wiedziałby, co poszło do gry. */
        if (stan.seats === 3) {
        ...
        for (let m = 0; m < stan.seats; m++) {
          if (stan.playerControllers[stan.activePlayers[m]] !== 'bot') stan.czekamNaMusik.push(m);
        }
      }
      const jeszcze = stan.czekamNaMusik || [];
      if (jeszcze.length) {
        czekajNa(stan, jeszcze[0], 'musikok', { czekaja: jeszcze.slice() });
        return stan;
      }
      stan.phase = 'exchange';
    ...
    if (stan.phase === 'giftreveal') {
      /* Czekamy, aż KAŻDY obdarowany potwierdzi, że obejrzał swoją kartę. Odliczanie było
         zawodne: wystarczyło na moment odwrócić wzrok, żeby przegapić, co się dostało. */
      const doPotwierdzenia = (stan.czekamNaDary || []).filter(m => {
        const rid = stan.activePlayers[m];
        return stan.playerControllers[rid] !== 'bot';
      });
      if (doPotwierdzenia.length) {
        czekajNa(stan, doPotwierdzenia[0], 'dar', { czekaja: doPotwierdzenia.slice() });
        return stan;
      }
...
function wezMusik(stan, ktory) {
  ...
  /* Także w pojedynku pokazujemy odkryty musik obu graczom, zanim ruszy odkładanie kart. */
  stan.phase = 'musikreveal';
  stan.musikPokazany = false;
...
function botOddajeKarty(stan) {
  ...
  if (stan.seats === 3) {
    stan.phase = 'giftreveal';
    stan.czekamNaDary = Object.keys(stan.receivedCards).map(Number);
  } else {
    stan.phase = 'bombchance';
  }
}
...
function wykonaj(stan, rid, akcja, rand) {
  ...
  /* Przy oglądaniu rozdanych kart czeka kilka osób naraz, więc dopuszczamy każdą z nich. */
  const wieluNaraz = (o.typ === 'dar' || o.typ === 'musikok')
                   && Array.isArray(o.czekaja) && o.czekaja.indexOf(miejsce) !== -1;
  if (miejsce !== o.seat && !wieluNaraz) return { ok: false, why: 'To nie Twoja kolej.' };
  ...
  switch (o.typ) {
    ...
    case 'musikok': {
      stan.czekamNaMusik = (stan.czekamNaMusik || []).filter(m => m !== miejsce);
      break;
    }
    case 'dar': {
      /* Potwierdzenie przyjmujemy od każdego obdarowanego z osobna. */
      stan.czekamNaDary = (stan.czekamNaDary || []).filter(m => m !== miejsce);
      break;
    }
    case 'karta': {
