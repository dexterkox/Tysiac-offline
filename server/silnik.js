...
function podliczRozdanie(stan) {
  ...
  for (let miejsce = 0; miejsce < n; miejsce++) {
    ...
    wiersze.push({ seat: miejsce, rid, lewy, meld, razem, delta, opis, total: stan.scores[rid] });
  }
  ...
  while (bezpiecznik++ < 500) {
    ...
    if (stan.phase === 'declare') {
      ...
      czekajNa(stan, stan.bidder, 'kontrakt', {
        min: stan.bidAmount,
        max: maksymalnaOdzywka(stan.hands[stan.bidder])
      });
      return stan;
    }
    ...
    if (stan.phase === 'play') {
      ...
      if (stan.trick.length === stan.seats) { rozstrzygnijLewe(stan); continue; }
      if (botem(stan, naRuchu)) { zagraj(stan, naRuchu, botKarta(stan, naRuchu)); continue; }
...
function rzucBombe(stan) {
  ...
  for (let m = 0; m < stan.seats; m++) {
    ...
    stan.scores[r] += delta;
    wiersze.push({ seat: m, rid: r, lewy: 0, meld: 0, razem: 0, delta, opis: m === stan.bidder ? 'bomba' : '', total: stan.scores[r] });
  }
  stan.summaryRows = wiersze;
...
function rozstrzygnijLewe(stan) {
  ...
  stan.history.push({ winner: miejsce, cards: stan.trick.map(t => t.card), no: stan.tricksPlayed + 1 });
  stan.trick = [];
  stan.leader = miejsce;
  stan.tricksPlayed++;
...
function wykonaj(stan, rid, akcja, rand) {
  ...
  if (o.typ === 'potwierdzenie') {
    ...
    stan.dealer = (stan.dealer + 1) % stan.playerCount;
...
function widokDla(stan, rid) {
  ...
  const odkryty = (w.phase === 'musikreveal' || w.phase === 'exchange' ||
...
module.exports = {
  ...
  nowaGra, rozdaj, krok, wykonaj, widokDla, cid
};
