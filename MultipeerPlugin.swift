...
public class MultipeerPlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    public let pluginMethods: [CAPPluginMethod] = [
        ...
        CAPPluginMethod(name: "checkNearbyPermissions",   returnType: CAPPluginReturnPromise),
    ...
    @objc func isSupported(_ call: CAPPluginCall) {
        /// `bluetoothOn` istnieje po to, żeby lista kontrolna w aplikacji wyglądała tak samo na
        /// obu systemach. iOS nie pozwala sprawdzić stanu Bluetootha bez wywołania pytania
        /// o uprawnienie, więc zgłaszamy „włączony" i pozwalamy systemowi zapytać w swoim czasie.
        call.resolve(["supported": true, "bluetoothOn": true])
    }

    /// iOS nie ma uprawnień, o które można zapytać z wyprzedzeniem — pytanie o sieć lokalną
    /// pokazuje sam system przy pierwszej próbie wyszukiwania. Zgłaszamy więc komplet zgód,
    /// żeby bramka w aplikacji wpuściła gracza dalej, a systemowe okno pojawiło się później.
    @objc func checkNearbyPermissions(_ call: CAPPluginCall) {
        call.resolve(["granted": true, "missing": [String]()])
    }
    ...
    @objc func send(_ call: CAPPluginCall) {
        ...
        if let peer = call.getString("peer") {
            targets = session.connectedPeers.filter { $0.displayName == peer }
            if targets.isEmpty {
                call.resolve(["sent": false])
                return
            }
        ...
        do {
            try session.send(Data(text.utf8), toPeers: targets, with: .reliable)
